"""First-party averaged structured-perceptron UPOS research pilot."""
...
def lexicon_hash(form: str) -> int:
    """Separate full-width identity hash used only to reject word collisions."""
...
def feature_ids(forms: Sequence[str], index: int, buckets: int) -> tuple[int, ...]:
    """Return the one predeclared hashed lexical/morphological feature family."""
    ...
    # Preserve multiplicity after hashing: two distinct templates that collide
    # contribute twice, just as they did before hashing.
...
class Model:
    ...
    def decode_indices(self, forms: Sequence[str]) -> tuple[int, ...]:
        """Exact 17-tag Viterbi with lexicographic ties and scored EOS."""
...
def _decode_indices(
    ...
    """Decode without constructing/validating a model during every update."""
