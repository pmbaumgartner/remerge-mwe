...
root = Path("tests/performance/reference_corpus")
docs = [txt.read_text() for txt in sorted(root.glob("*.TXT"))]
...
for label, kwargs in configs:
    ...
    print(
        f"{label:14s} avg={mean(samples):8.3f}s min={min(samples):8.3f}s "
        f"max={max(samples):8.3f}s first={first_winner}"
    )
PY
